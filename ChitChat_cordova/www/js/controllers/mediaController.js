angular.module('spartan.media', [])
	...
	.controller('FileController', function ($rootScope, $scope, $ionicLoading, sharedObjectService, roomSelected) {
		...
		$scope.uploadFile = function (id) {
			...
			if (file !== undefined) {
				var reader = new FileReader();
				reader.onloadend = function () {
					var file = new UploadMediaWeb($ionicLoading, sharedObjectService, roomSelected.getRoom()._id, reader.result, ContentType[ContentType.File], function (id, messageId) {
		...
		$scope.uploadImage = function (id) {
			if (ionic.Platform.platform() === "ios") {
				...
				if (file !== undefined) {
					var reader = new FileReader();
					reader.onloadend = function () {
						var img = new UploadMediaWeb($ionicLoading, sharedObjectService, roomSelected.getRoom()._id, reader.result, ContentType[ContentType.Image], function (id, messageId) {
							$scope.$emit('delectTemp', [id]);
						});
						mediaUpload[id] = img;
						mediaUpload[id].upload();
		...
		$scope.uploadVideo = function (id) {
			if (ionic.Platform.platform() === "ios") {
				...
				if (file !== undefined) {
					var reader = new FileReader();
					reader.onloadend = function () {
						var video = new UploadMediaWeb($ionicLoading, sharedObjectService, roomSelected.getRoom()._id, reader.result, ContentType[ContentType.Video], function (id, messageId) {
							$scope.$emit('delectTemp', [id]);
						});
						mediaUpload[id] = video;
						mediaUpload[id].upload();
	...
	.controller('VoiceController', function ($rootScope, $scope, $ionicLoading, $cordovaProgress, $timeout, $cordovaFileTransfer, $cordovaFile,
		...
		$scope.uploadVoice = function (id) {
			if (ionic.Platform.platform() === "ios") {
				...
				var voice = new UploadMediaWeb($ionicLoading, sharedObjectService, roomSelected.getRoom()._id, id, ContentType[ContentType.Voice], function (id, messageId) {
					$scope.$emit('delectTemp', [id]);
				});
				mediaUpload[id] = voice;
				mediaUpload[id].upload();
...
function UploadMediaWeb($ionicLoading, sharedObjectService, rid, uri, type, callback) {
